...
<div class="dashboard container">
    ...
    <div class="collection-filters">
        ...
        <div class="filter-group">
            ...
            <select id="sort-select" class="filter-select">
                ...
                <option value="innovation-desc" data-en="Innovation (High-Low)" data-ko="혁신 (높음-낮음)">Innovation / 혁신</option>
            </select>
        </div>
    ...
    <section class="collection-section">
        <div class="grid-container" id="strategies-grid">
            {% for strat in site.strategies %}
            <a href="{{ strat.url | relative_url }}"
               class="item-card"
               data-era="{{ strat.era | default: 'modern' }}"
               data-year="{{ strat.year | default: 0 }}"
               data-name-en="{{ strat.title_en | downcase }}"
               data-name-ko="{{ strat.title_ko }}"
               data-innovation="{{ strat.metrics.innovation_index | default: 0 }}">
                <div class="card-image">
                    ...
                    <div class="placeholder-image strategy-placeholder">
                        ...
                        <span class="placeholder-text lang-en-only">{{ strat.title_en }}</span>
                        <span class="placeholder-text lang-ko-only">{{ strat.title_ko }}</span>
                    </div>
                    {% endif %}
                </div>
                <div class="card-content">
                    <h3 class="card-title-en lang-en-only">{{ strat.title_en }}</h3>
                    <p class="card-title-ko lang-ko-only">{{ strat.title_ko }}</p>
                    <p class="card-summary lang-en-only">{{ strat.summary_en }}</p>
                    <p class="card-summary lang-ko-only">{{ strat.summary_ko }}</p>
                    {% if strat.metrics %}
                    <div class="card-metrics">
                        <span class="metric-badge" title="Innovation">
                            💡 {{ strat.metrics.innovation_index | default: "N/A" }}
                        </span>
                        <span class="metric-badge" title="Communication">
                            📡 {{ strat.metrics.communication_efficiency | default: "N/A" }}
                        </span>
                    </div>
                    {% endif %}
                </div>
            ...
            <div class="empty-state">
                <p class="lang-en-only">No strategies in the archive yet. Check back soon!</p>
                <p class="lang-ko-only">아카이브에 아직 전략이 없습니다. 곧 다시 확인하세요!</p>
            </div>
            {% endif %}
        </div>
    </section>
    ...
    function filterAndSort() {
        ...
        visibleCards.sort((a, b) => {
            switch (sortMethod) {
                ...
                case 'innovation-desc':
                    return parseInt(b.dataset.innovation) - parseInt(a.dataset.innovation);
                default:
                    return 0;
            }
        });
